...
$(document).ready(function(){      
	...
	function init_template(){        
            ...
            $('[data-toggle-theme], [data-toggle-theme-switch]').off().on('click',function(){
                ...
                $('body').toggleClass('theme-light theme-dark');
                    ...
                    if($('body').hasClass('theme-light')){
                        ...
                        localStorage.setItem('React⚡React⚡AppKit-Theme','light-mode')
                    }
                    if($('body').hasClass('theme-dark')){
                        darkSwitch.prop('checked', true); 
                        localStorage.setItem('React⚡React⚡AppKit-Theme','dark-mode')
                    }
                setTimeout(function(){allowTransitions();},150);
                return false;
            })
            if (localStorage.getItem('React⚡React⚡AppKit-Theme') == "dark-mode") {darkSwitch.prop('checked', true); $('body').removeClass('theme-light').addClass('theme-dark');}
            if (localStorage.getItem('React⚡React⚡AppKit-Theme') == "light-mode") {darkSwitch.prop('checked', false); $('body').removeClass('theme-dark').addClass('theme-light');}
        ...
        localStorage.setItem('z_React⚡React⚡AppKit-Visited:' + window.location.pathname, 'Page Visited');
        $.fn.visited = function (className) {
            className = className || false;
            $(this).each(function (index, element) {
                if (element.host == window.location.host &&
                    localStorage.getItem('z_React⚡React⚡AppKit-Visited:' + element.pathname) ||
                    localStorage.getItem('z_React⚡React⚡AppKit-Visited:' + element.pathname + '/')) {
                    if (className) {
        ...
        $('.list-group a').on('click',function(){if($(this).attr('href') != '#'){localStorage.setItem('z_React⚡React⚡AppKit-Visited-Scroll-Position', $(window).scrollTop());}})
        if($('.check-visited').length){
            setTimeout(function(){
                var getScroll = localStorage.getItem('z_React⚡React⚡AppKit-Visited-Scroll-Position');
                $(window).scrollTop(getScroll)
                 localStorage.removeItem('z_React⚡React⚡AppKit-Visited-Scroll-Position');
            },200);
        }
        ...
        if(visitDetection.length){
            var lastVisitValue = localStorage.getItem('React⚡React⚡AppKit-Last-Visited');
            var d = new Date();
            ...
            if(!lastVisitValue){
                $('.never-visited').show();
                $('.before-visited').hide();
                localStorage.setItem('React⚡React⚡AppKit-Last-Visited', '<br>Your last visit was ' + strDate +' at '+ strTime)
            } else {
       ...
       $(".text-size-default").click(function() {$(".text-size-changer *").css("font-size", "");});
        ...
        function highlightLoader(){
            $('[data-highlight-style]').on('click',function(){
                ...
                localStorage.setItem('React⚡React⚡AppKit-Highlight', getHighlight);
                $('#highlight').remove();
                $('head').append( $('<link rel="stylesheet" id="highlight" type="text/css" />').attr('href', getHighlight) );
                return false;
            });
            var rememberHighlight = localStorage.getItem('React⚡React⚡AppKit-Highlight');
        ...
        if(pwaEnabled === true){
            ...
            var dismissDate = localStorage.getItem('React⚡React⚡AppKit-PWA-Timeout-Value');
            if((day - dismissDate) > pwaRemind){
                localStorage.removeItem('React⚡React⚡AppKit-PWA-Prompt')
            } 
                
            //Dismiss Prompt Button
            $('.pwa-dismiss').on('click',function(){
                console.log('User Closed Add to Home / PWA Prompt')
                localStorage.setItem('React⚡React⚡AppKit-PWA-Prompt', 'install-rejected');
                $('body').find('#menu-install-pwa-android, #menu-install-pwa-ios, .menu-hider').removeClass('menu-active'); 
                localStorage.setItem('React⚡React⚡AppKit-PWA-Timeout-Value', day);
            });
            ...
            if (isMobile.Android()) {
                function showInstallPrompt(){
                    if($('#menu-install-pwa-android, .add-to-home').length){
                        if (localStorage.getItem('React⚡React⚡AppKit-PWA-Prompt') != "install-rejected") {
                            setTimeout(function(){
